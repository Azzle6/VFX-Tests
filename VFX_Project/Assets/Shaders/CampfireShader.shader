	...
	SubShader
	{
		...
		Pass
		{
					...
					)
			{
				...
				float4 color14 = IsGammaSpace() ? float4(1,0.6624398,0.1084906,0) : float4(1,0.3963504,0.01139139,0);
				float4 color17 = IsGammaSpace() ? float4(0.6981132,0.1571927,0.009878943,0) : float4(0.4453062,0.02129841,0.000764624,0);
				float lerpResult89 = lerp( temp_output_16_0 , tex2DNode18.r , 0.6);
				float smoothstepResult90 = smoothstep( 0.3 , 0.8 , lerpResult89);
				float4 lerpResult13 = lerp( color14 , color17 , smoothstepResult90);
				float4 temp_output_12_0 = ( smoothstepResult19 * lerpResult13 );
				
				float smoothstepResult117 = smoothstep( 0.0 , 0.4 , smoothstepResult19);
				
				surfaceDescription.Color = temp_output_12_0.rgb;
				surfaceDescription.Emission = temp_output_12_0.rgb;
				surfaceDescription.Alpha = smoothstepResult117;
